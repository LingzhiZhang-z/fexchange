...
from fexchange.core.fock_basis import N_ORB, make_basis_id
from fexchange.core.space_ls import ELL, build_space_ls_matrix
...
def build_lsjm(
    ...
    for key, idxs in term_indices.items():
        ...
        if hint_rank_matrix_map is not None:
            H_int_blk = V_blk.conj().T @ hint_rank_matrix_map[2] @ V_blk
            off_int = H_int_blk - np.diag(np.diag(H_int_blk))
            max_int = float(np.max(np.abs(off_int))) if off_int.size else 0.0
            if max_int > EPS_DIAG:
                raise NumError(
                    "FXE-NUM-001",
                    f"LSJM H_int block leakage for term {key}: {max_int:.3e} > eps_diag={EPS_DIAG:.1e}",
                    module="lsjm",
                    level="LSJM",
                    actual={
                        "residual_name": "r_diag_hint_lsjm",
                        "residual_value": max_int,
                        "threshold": EPS_DIAG,
                        "term": {"alpha": key[0], "L": key[1], "twoS": key[2]},
                    },
                )
