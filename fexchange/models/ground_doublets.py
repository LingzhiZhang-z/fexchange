...
from fexchange.core.symmetry import analyze_cef_symmetry, build_symmetry_metadata, irrep_dimension
from fexchange.io.matrix_io import load_matrix_file
from fexchange.models.hcef import build_hcef_matrix_J
from fexchange.representations.lsjm import select_soc_lowest_subspace
...
def load_or_build_W(
    ...
    *,
    ...
    sources = cfg["sources"]
    mode = sources["kramer_source"]
    if mode == "file":
        path = Path(cfg["paths"]["kramer_file"])
        W = load_matrix_file(path, preferred_key="W")
        ...
        check_orthonormal(W, label="W_file", module="projection")
        return {
            "W": W,
            "kramer_basis_id": "file_input_v1",
    ...
    if mode != "cef":
        raise InputError(
            "FXE-INPUT-003",
            f"Unknown kramer_source: {mode!r}",
            actual={"kramer_source": mode},
        )
