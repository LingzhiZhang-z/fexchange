...
- Any operator implementation must be traceable to `basis_id_from`/`basis_id_to` and bit rules from `./standards/en/01-core/01-00-FOCK_SLATER.md`.

## 2) Implementation Levels (MUST)
The reference implementation may use either of two routes:
1. Matrix-first route: determinant actions plus direct matrix assembly.
2. Structured-term route: primitive ops, monomials, and operator objects.

The current project uses the matrix-first route as the primary contract.
...
matrix-first:
  ...
  one_body_operator_matrix(h_pq, ...) -> NDArray
  two_body_operator_matrix(V_pqrs, ...) -> NDArray

structured-term (optional):
  PrimitiveOp  = {kind: "cdag"|"c", p: int}
  Monomial     = {coef: complex, ops: list[PrimitiveOp]}
...
- If the structured-term route is used, monomial operator order must be explicit
  and preserved.

## 3) Canonical Ordering and Simplification (Optional Structured-Term Route)
...
Validation:
- If canonicalization is implemented, it must be deterministic.
- Zero-coefficient terms must be dropped using fixed tolerance.

## 3.1) Two-Body Index Canonical Rule (Optional Structured-Term Route)
...
- Bitstring action follows `./standards/en/01-core/01-00-FOCK_SLATER.md`.
...
- Intermediate variables:
  determinant actions, direct matrix fills, and optionally expanded monomial
  lists / temporary hash maps.
- Output variables:
  many-body dense/sparse matrix in the target sector; a canonical `Operator`
  object is optional.

Code form:
```text
build_operator_matrix(inputs) -> NDArray | sparse_matrix
# optional:
...
- This project does not require a universal serialized operator exchange format.
- Term-list JSON and sparse-matrix caches are optional representations.
- When an operator is serialized, basis metadata must remain explicit.

Code form:
```text
optional JSON fields:
  schema, name, basis_id_from, basis_id_to, orbital_order_id, n_orb,
  sector_from, sector_to, terms=[...]

optional matrix-cache fields:
  row, col, data_re, data_im, shape,
  basis_id_from, basis_id_to, orbital_order_id, sector_from, sector_to, name
```

Validation:
- If JSON term lists are emitted, their ordering must be stable after
  canonicalization.
- Serialized matrix caches without matching metadata must be rejected.
