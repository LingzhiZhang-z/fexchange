...
- Public interface exchange MUST use `StateSet` uniformly.
- A single exported state must be encoded as `StateSet` with `n_states = 1`.
...
- Determinant ordering is inherited from `./standards/en/01-physics/01-00-FOUNDATIONS_FOCK_SLATER.md`.
...
Validation:
- `basis_id`, `n_orb`, `n_ele`, and determinant order tag must match before algebra.
...
- Required metadata: `schema`, `basis_id`, `n_orb`, `n_ele`, `state_order_id`, `unit`, `labels`, `meta`.
- `state_order_id` must be explicit and stable.
...
Validation:
- Missing required metadata must fail loading.
