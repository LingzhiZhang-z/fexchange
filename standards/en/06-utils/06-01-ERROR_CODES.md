...
- Every failure payload must contain:
  ...
  - `paths`
  - `ts_utc`
- `module`, `level`, `stage`, `op`, `run_id`, and `key` are optional context
  fields. They may be omitted or left empty when the caller has no stable value.
- `expected/actual/paths` can be empty objects but must exist.

Code form:
```text
payload = {
  code, message, schema_version, standard_version,
  expected: {...}, actual: {...}, paths: {...},
  ts_utc,
  module?, level?, stage?, op?, run_id?, key?
}
```
...
## 6) Failure-Context Minimum per Domain (Recommended)
Recommended:
...
Validation:
- When such domain-specific context is available, it should be carried in the
  payload.
