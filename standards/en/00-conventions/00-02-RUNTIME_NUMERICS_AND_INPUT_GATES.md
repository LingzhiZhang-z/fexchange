...
- Degenerate-cluster gauge must be fixed by deterministic pivot-phase rule from `./standards/en/01-physics/01-01-STATE_VECTOR_CONVENTION.md`.
...
MUST:
- State phase follows `./standards/en/01-physics/01-01-STATE_VECTOR_CONVENTION.md` pivot rule.
...
Validation:
- Runtime metadata must record selected path and chunk policy.
...
MUST:
- Every stage output must include active numerical profile.
...
Validation:
- Missing `numerics_meta` is a contract violation.
