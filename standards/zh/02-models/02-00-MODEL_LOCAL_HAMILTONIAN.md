...
Index:
- `n`：固定电子数扇区电子数。
- `O2/O4/O6`：分秩库仑算符（见 `./standards/en/02-models/02-01-HINT_FORM.md`）。
...
MUST:
- 使用单一 SOC 强度参数 `zeta`。
- 算符形式以 `./standards/en/02-models/02-02-HSOC_FORM.md` 为准。
...
MUST:
- 使用 Stevens 算符展开。
- 仅允许 `./standards/en/02-models/02-03-HCEF_FORM.md` 中定义的对称分支。
